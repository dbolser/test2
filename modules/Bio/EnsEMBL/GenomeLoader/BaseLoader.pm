...
# Reference to an array of the one and only parent-level location of the iobject or its sub-locations (if there are any).
  ...
  if ($iobject_location->{strand} == 1) {
	  ...
	  my $ilocation (sort { $a->{min} <=> $b->{min} } @{$ilocations})
	{
	  ...
	  {
		$pep_end = int(
		   ($seq_end - $ilocation->{min} + $seq_translated + $offset)/3)
		  + 1;
	  }
...
# Peptide coord is outside coding region  But not in a gap between coding regions
						   ...
						   @{$ilocations})
	{
	  ...
	  {
		$pep_start = int(
		   ($ilocation->{max} - $seq_end + $seq_translated + $offset)/3)
		  + 1;
	  }
	  if (!$pep_end &&
		  $seq_start >= $ilocation->{min} &&
...
sub store_mapping_path {
  ...
  my $validate = sub {
	...
	$seen_ranks{$rank} &&
	  croak('CoordSystem ' . $_[0]->name .
			" shares rank $rank with " . $seen_ranks{$rank}->name);
	$seen_ranks{$rank} = $_[0];
  };
  @csystems =
