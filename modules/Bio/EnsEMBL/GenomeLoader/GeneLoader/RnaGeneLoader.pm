...
sub load_gene {
  ...
  foreach my $xref ( @{ $igene->{xrefs} } ) {
    ...
    if ( $ensembl_dbname eq "ENA_FEATURE_GENE" ) {
      my $txref = { "databaseReferenceType" =>
...
# (ie. ensembl rank is opposite to Integr8 rank for reverse strand).
      ...
      foreach my $itranscript_location ( @{$ilocations} ) {
             ...
             $itranscript_location->{max} + 1 )
        {
          $self->log()->warn(
              ...
              $egene->stable_id . '/' . $itranscript->{analysis} . '/' .
                $egene->biotype . ' exon=' .
        ...
        if ( defined $inserts ) {
          for my $i ( @{$inserts} ) {
            ...
            } ## end if ( $i->{start} >= $itranscript_location...)
      ...
      $self->set_stable_id(
                     { identifyingId => $itranscript->{identifyingId} },
...
sub get_xref_for_name {
  ...
  my $display_xref =
    Bio::EnsEMBL::DBEntry->new( -DBNAME     => $db_name,
  ...
  if ( !$display_xref ) {
    $self->log()
      ->warn( "No display_xref found for RNA gene of type " .
           $egene->biotype() . "/" . $egene->analysis()->logic_name() );
  }
