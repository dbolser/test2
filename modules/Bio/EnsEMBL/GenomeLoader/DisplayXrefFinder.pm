...
sub new {
  ...
  $self->{gl_name_xref} =
    $self->plugins()->get("analysisFinder")
    ->get_analysis_by_name('gl_name_xref');
  return $self;
}
...
sub _initialize {
  ...
  my $xrefs_search = $args{display_xrefs_search};
  if ( !$xrefs_search ) {
    $xrefs_search = { NAMES()->{GENE} => [
  ...
  $self->description_sources( { db_name => XREFS()->{UNIPROT_SWISSPROT},
...
sub get_db_display_xref {
  ...
  my @refs =
    grep { $_->dbname() eq $display_xref_source }
...
sub get_xref_type_for_gene {
  ...
  if ( !$type ) {
    croak( "Could not find gene type for gene of biotype " .
           $eobj->biotype() . " with analysis type " .
           $eobj->analysis()->{_logic_name} );
  }
  return $type;
}
...
sub get_xref_type_for_transcript {
  ...
  if ( !$type ) {
    croak( "Could not find transcript type for transcript of biotype " .
           $eobj->biotype() . " for analysis type " .
           $eobj->analysis()->{_logic_name} );
  }
  return $type;
}
