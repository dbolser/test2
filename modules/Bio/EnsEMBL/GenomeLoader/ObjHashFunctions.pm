...
use Exporter 'import';
our @EXPORT_OK = qw(get_gene_hash get_type_analysis_hash get_location_hash get_gene_xref_hash get_gene_display_xref_hash);
my $HASH_SEP = ';';

sub get_gene_hash {
	my ( $feat, $hash ) = @_;
	$hash = build_hash( $feat->external_name(), $feat->description(),
		get_type_analysis_hash( $feat, $hash ) );
	foreach my $ts (sort {$_[0]->start() <=> $_[1]->start()} @{$feat->get_all_Transcripts()}) {
...
sub get_gene_display_xref_hash {
	my ( $feat, $hash ) = @_;
	return build_hash( $feat->display_xref()->primary_id(),
		get_type_analysis_hash( $feat, $hash ) );
}
...
sub get_location_hash {
	my ( $feat, $hash ) = @_;
	return build_hash( $feat->start(), $feat->end(), $feat->strand(), $hash );
}
