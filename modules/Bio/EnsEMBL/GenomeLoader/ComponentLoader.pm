...
sub load_assembly {
  ...
  for my $component (@{$components}) {
	$self->log()
	  ->info("Storing sequence for component " .
			 $component->{name} . "/" . $component->{accession});
  ...
  for my $component (@{$components}) {
	...
	{
	  $self->log()
		->info("Storing assembly for component " .
			   $component->{name} . "/" . $component->{accession});
	  for my $ass (@{$component->{assembly}}) {
		...
		if (!defined $ass_comp) {
		  croak "Assembly component " .
			$ass->{accession} . " not stored";
		}
		...
		my $map_str = $ass_slice->coord_system_name() .
		...
		my $ctg_slice =
		  $sa->fetch_by_region($ass_comp_slice->coord_system_name(),
							   ...
							   $ass->{location}{strand});
		$self->log()
		  ->info("Storing assembly from " .
				 $slice->name() . " to " . $ctg_slice->name());
